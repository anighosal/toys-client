...
const AllToysCard = ({ toys }) => {
  ...
  return (
    <tr className="shadow-xl border border-gray-300  text-white rounded-md mt-4 ">
      ...
      <td className="w-1/5 md:w-1/6">{name}</td>
      <td className="w-1/5 md:w-1/6">{subCategory}</td>
      <td className="w-1/5 md:w-1/6">$ {price}</td>
      <td className="w-1/5 md:w-1/6">{quantity}</td>
      <td className="text-yellow-400 w-1/5 md:w-1/6">{rating}</td>

      {/* Move the seller column to the right side */}
      <td className="hidden md:table-cell w-1/5 md:w-1/6">{seller}</td>

      <th className="w-1/5 md:w-1/6">
        <Link to={`/toy/${_id}`}>
